...
//	3. 요청을 처리할 적당한 이름의 메소드를 만든다. 뷰 페이지 이름은 문자열이므로 메소드의 리턴 타입을
//	   String으로 지정한다.
...
public class MyController {
	...
	public String view() {
		 System.out.println("컨트롤러의 view() 메소드 실행");
		logger.info("컨트롤러의 view() 메소드 실행");
		// 필요한 작업
...
//		addAttribute(key, value): Model 인터페이스 객체에 key에 따른 value를 넣어준다.
		...
		// "/WEB-INF/views/" + "model/model" + ".jsp" => "/WEB-INF/views/model.jsp"
		return "model/model";
	}
	
	@RequestMapping("/modelAndView")
	// ModelAndView 클래스 객체는 뷰 페이지의 이름과 뷰 페이지로 넘겨줄 데이터를 저장한다.
	// 뷰 페이지 이름과 뷰 페이지로 넘겨줄 데이터를 ModelAndView 클래스 객체에 저장해서 리턴시켜야
	// 하므로 메소드의 리턴타입에 ModelAndView를 지정한다.
