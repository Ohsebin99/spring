...
public class MainClass {
	public static void main(String[] args) {
		...
		// ConfigurableApplicationContext 인터페이스 타입의 객체에서 DI컨테이너 환경 설정 정보를
		// getEnvironmemt()메소드로 얻어와서 ConfigurableEnvironment 인터페이스 객체에 저장한다.
		ConfigurableEnvironment env = ctx.getEnvironment();
		// properties 정보를 저장하는 MutablePropertySources클래스 타입의 객체에
		// ConfigurableEnvironment 인터페이스 타입의 객체에서 getPropertySources()메소드로
		// properties 정보만 얻어와 저장한다.
		MutablePropertySources mutablePropertySources = env.getPropertySources();
		
		try {
			// DI 컨테이터에서 얻어온 properties 정보가 저장되는 MutablePropertySources 클래스 객체
			// mutablePropertySources에 addLast() 메소드를 사용해서 맨 마지막 위치에
			// admin.proterties파일의 properties내용을 읽어 추가한다.
		...
		System.out.println("======================================================================");
		// AdminConnection 클래스의 bean이 생성된 후 환경설정 정보에 저장된 admin.proterties파일의 정보를
		// 넘겨준다.
		System.out.println("AdminConnection 클래스의 bean이 생성된 후 환경설정 정보에 저장된 properties파일의 정보를 넣어준다.");
