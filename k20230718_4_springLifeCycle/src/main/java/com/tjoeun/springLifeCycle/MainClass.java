...
public class MainClass {
	public static void main(String[] args) {
		...
		*/
		...
		// 빈 컨테이너만 생성되고 bean 설정 정보를 들어있지 않다.
		GenericXmlApplicationContext ctx = new GenericXmlApplicationContext();
		
		
		// load() 메소드로 xml 파일에서 정의한 bean 설정 정보를 DI 컨테이너에 넣어준다.
		// load() 메소드는 DI 컨테이너에 bean 설정 정보를 넣어주기만 하기 때문에 아직 bean이 생성되지 
		// 않은 상태이다. => 생성자가 실행되지 않은 상태이다.
		ctx.load("classpath:applicationCTX.xml"); // bean 설정 정보를 넣어준다.
		
		// load() 메소드를 사용해서 bean 설정 정보를 DI 컨테이너에 넣어준 경우 bean이 생성되는 시점이
		// 2가지이다.
		// 1. refresh() 메소드가 실행되는 순간 load() 메소드로 DI 컨테이너에 넣어준 bean 설정 정보에 따른
		// 		bean이 일괄적으로 생성한다.
		// 2. refresh() 메소드를 실행하지 않으면 getBean() 메소드로 DI 컨테이너에 해당 bean을 얻어오는 
		//		순간 bean이 생성된다.
		...
		// refresh() 메소드를 실행했더라도 close() 메소드를 실행하지 않으면 destroy() 메소드는 실행되지 
		// 않는다.
		// close() 메소드를 실행더라고 refresh() 메소드를 실행하지 않았다면 destroy() 메소드는 실행되지
		// 않는다.
		ctx.close();
	}
}
