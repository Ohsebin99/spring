...
//	리턴하는 내용 중에서 필드에 저장할 데이터의 key를 @Value 어노테이션에 ${}를 사용해서 @Value
//	어노테이션이 설정된 필드에 저장한다.
	@Value("${admin.id}")
...
//	실행될 때 자동으로 실행되는 메소드로 properties 파일의 데이터를 읽어서 필드를 초기화할 준비를
//	하는 메소드 => 반드시 정적(static) 메소드로 만들어야 한다.
	@Bean
	public static PropertySourcesPlaceholderConfigurer configurer() {
		...
		// Resource 인터페이스 타입의 변수의 ClassPathResource 클래스의 생성자로 읽어들일 properties
		// 파일의 이름을 넘겨서 properties 파일의 내용을 읽는다.
		Resource resource = new ClassPathResource("admin.properties");
		// Resource 인터페이스 탕비의 변수로 읽어들인 properties 파일의 내용을 setLocation() 메소드로
