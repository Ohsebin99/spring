...
// JdbcTemplate 클래스 타입의 객체를 생성하고 넣어줘야 한다.
   ...
   public void setTemplate(JdbcTemplate template) {
	...
	// 사용할 수 있게 하기 위해서 적당한 이름의 패키지에 적당한 이름의 클래스를 만들고 적당한 이름으로
	// 작성한 클래스의 정적 필드에 servlet-context.xml 파일에서 생성시 컨트롤러의 JdbcTemplate 클래스
	// 객체에 저장된 bean을 넣어준다.
	
	
	// 적당한 이름을 클래스를 만들고 JdbcTemplate 객체를 static으로 선언한 후 코딩한다.
   ...
   public String increment(HttpServletRequest request, Model model) {
	   ...
	   // 컨트롤러에 "/increment"로 요청하는 페이지에서 넘어오는 클릭한 글의 글 번호와 그 글이 있는
	   // 페이지 번호가 저장된 HttpServletRequest 인터페이스 객체에 저장한다.
	   model.addAttribute("request", request);
	   ...
	   // 조회수를 증가시킨 글 번호와 작업후 돌아갈 페이지 번호를 Model 인터페이스 객체에 넣어준다.
...
//  조회수를 증가시킨 글 1건을 얻어오는 메소드
   ...
   public String contentView(HttpServletRequest request, Model model) {
	   ...
	   //   컨트롤러에 "/contentView"로 요청하는 페이지에서 넘어오는 조회수를 증가한 글의 
	   //	글번호와 그 글이 있는 페이지 번호가 저장된
...
//   글 1건을 삭제하는 메소드
   ...
   public String delete(HttpServletRequest request, Model model) {
	...
	//   컨트롤러에 "/delete"로 요청하는 페이지에서 넘어오는 삭제할 글번호가 저장된 HttpServletRequest
	//   HttpServletRequest 인터페이스 객체를 Model 인터페이스 객체에 저장한다.
	   model.addAttribute("request", request);
...
//  답글을 입력하기 위해 브라우저에 출력할 메인글을 얻어오고 답글을 입력하는 페이지를 호출하는 메소드
   ...
   public String reply(HttpServletRequest request, Model model) {
	   ...
	   //   컨트롤러에 "/reply"로 요청하는 페이지에서 넘어오는 답변을 입력할 글의 
	   //	메인글의 글번호가 저장된 HttpServletRequest 인터페이스 객체를 Model 
	   //	인터페이스 객체에 저장한다.
	   model.addAttribute("request", request);
